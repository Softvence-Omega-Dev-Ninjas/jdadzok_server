...
export class CapLevelService {
  ...
  async calculateCapEligibility(userId: string): Promise<CapEligibilityResult> {
    ...
    for (let i = currentLevelIndex + 1; i < this.capLevelOrder.length; i++) {
      ...
      const levelRequirements =
        await this.repository.getCapRequirements(checkLevel);
  ...
  async promoteUserCapLevel(
    ...
    if (!requirements) {
      throw new BadRequestException(
        `Requirements not found for level: ${newLevel}`,
      );
    }
    ...
    if (requirements.requiresVerification && !bypassVerification) {
      throw new BadRequestException(
        "Admin verification required for this level",
      );
    }
    ...
    const updatedUser = await this.repository.updateUserCapLevel(
      userId,
      newLevel,
    );
  ...
  async getCapRequirements(
    capLevel: CapLevel,
  ): Promise<CapRequirements | null> {
  ...
  private async calculateProgressPercentage(
    ...
    if (totalRequirements === 0) {
      return currentLevel === this.capLevelOrder[this.capLevelOrder.length - 1]
        ? 100
        : 0;
    }
  ...
  async processPendingPromotions(
    ...
    const eligibleUsers =
      await this.repository.getUsersEligibleForPromotion(capLevel);
    const requirements = await this.repository.getCapRequirements(capLevel);

    if (!requirements) {
      throw new BadRequestException(
        `Requirements not found for level: ${capLevel}`,
      );
    }
    ...
    for (const user of eligibleUsers) {
      try {
        ...
        console.error(
          `Failed to promote user ${user.id} to ${capLevel}:`,
          error,
        );
        failed++;
      }
