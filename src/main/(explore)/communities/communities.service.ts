...
export class CommunitiesService {
  ...
  async findOne(communityId: string) {
    const community = await this.prisma.communities.findUnique({
      ...
      include: { sharedProfile: true, members: true },
    });
    if (!community) {
  ...
  async updateCommunity(
    ...
    const isExistCommunity = await this.prisma.communities.findUnique({
      where: { id: communityid },
    });
    if (!isExistCommunity) {
      throw new NotFoundException("Community is Not Found.");
    }
    const user = await this.prisma.communities.findFirst({
