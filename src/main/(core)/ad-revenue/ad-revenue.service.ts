...
export class AdRevenueService {
  ...
  async createRevenueShare(
    createDto: CreateRevenueShareDto,
  ): Promise<RevenueShareResponseDto> {
    try {
      ...
      await this.userMetricsService.addEarnings(
        createDto.userId,
        createDto.amount,
      );
      ...
      this.logger.error(
        `Failed to create revenue share: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async calculateMonthlyRevenue(
    ...
    try {
      ...
      this.logger.error(
        `Failed to calculate monthly revenue: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getUserRevenueHistory(
    ...
    try {
      ...
      this.logger.error(
        `Failed to get revenue history for user ${userId}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async getUserRevenueSummary(userId: string): Promise<UserRevenueSummaryDto> {
    try {
      ...
      const totalEarnings = revenueShares.reduce(
      ...
      const averageMonthlyEarnings =
        activeMonths > 0 ? totalEarnings / activeMonths : 0;
      ...
      this.logger.error(
        `Failed to get revenue summary for user ${userId}: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async getPlatformRevenueStats(): Promise<PlatformRevenueStatsDto> {
    try {
      ...
      const totalRevenue = allShares.reduce(
        (sum, share) => sum + share.amount,
        0,
      );
      ...
      this.logger.error(
        `Failed to get platform revenue stats: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  private async calculateRevenueDistributions(
    ...
    const requirementsMap = new Map(
      capRequirements.map((req) => [req.capLevel, req]),
    );
   ...
   */
  private groupDistributionsByLevel(
  ...
  > {
    ...
    for (const stat of statsByLevel) {
      ...
      result[capLevel] = {
        ...
        averageEarnings: stat._sum.amount
          ? stat._sum.amount / stat._count.userId
          : 0,
