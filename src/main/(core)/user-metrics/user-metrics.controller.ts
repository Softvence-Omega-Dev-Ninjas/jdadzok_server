...
export class UserMetricsController {
  ...
  async recalculateActivityScore(@Param("userId") userId: string) {
    try {
      const updatedMetrics =
        await this.userMetricsService.recalculateAndUpdateActivityScore(userId);
      const newScore =
        await this.userMetricsService.calculateActivityScore(userId);
  ...
  async getUserActivityAnalytics(@Param("userId") userId: string) {
    try {
      ...
      const currentRank =
        await this.userMetricsService.getUserActivityRank(userId);
      ...
      const activityBreakdown = {
        ...
        followers: {
          ...
          percentage:
            ((metrics.totalFollowers * 0.5) / currentScore) * 100 || 0,
        },
        volunteerHours: {
  ...
  @ApiOperation({
    ...
    description:
      "Returns top users by activity score with rankings and cap levels",
  })
  @ApiQuery({
    name: "limit",
    required: false,
  ...
  async getActivityLeaderboard(
    ...
    try {
      ...
      const leaderboard: ActivityLeaderboardDto[] = topUsers.map(
        (metrics, index) => ({
          rank: index + 1,
  ...
  @ApiOperation({
    ...
    description:
      "Returns users with activity scores above a specified threshold",
  })
  @ApiQuery({
    name: "minScore",
    required: false,
  ...
  @ApiOperation({
    ...
    description:
      "Find users based on various metrics filters and sorting options",
  })
  @ApiResponse({
    status: HttpStatus.OK,
  ...
  async batchRecalculateMetrics(
    @Body(ValidationPipe) batchDto: BatchMetricsRecalculationDto,
  ) {
    try {
      const {
        userIds,
      ...
      if (!userIds || userIds.length === 0) {
        ...
        const highActivityUsers =
          await this.userMetricsService.getUsersWithHighActivity(0);
      ...
      // Process in batches
      await this.userMetricsService.bulkRecalculateActivityScores(
        targetUserIds,
      );
  ...
  @ApiOperation({
    ...
    description:
      "Returns comprehensive statistics about user activity across the platform",
  })
  @ApiResponse({
    status: HttpStatus.OK,
  ...
  async getPlatformActivityStats() {
    try {
      ...
      const highActivityUsers =
        await this.userMetricsService.getUsersWithHighActivity(50);
  ...
  @ApiOperation({
    ...
    description:
      "Updates the point values for different activities used in score calculation",
  })
  ...
  async updateActivityScoreWeights(
    @Body(ValidationPipe) weightsDto: ActivityScoreWeightsDto,
  ) {
    try {
