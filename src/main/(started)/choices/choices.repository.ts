...
export class ChoicesRepository {
  constructor(private readonly prisma: PrismaService, private readonly userChoiceRepository: UserChoiceRepository) { }
  ...
  async createMany(data: Required<CreateChoiceDto[]>) {
    return await this.prisma.$transaction(async (tx) => {
      ...
      for (const choiceDto of data) {
        ...
        if (!choice) { choice = await this.create(tx, choiceDto) }

        if (!choice) throw new ConflictException("Fail to find or create new one");

        // Check if this user already has this choice
        const existingUserChoice = await this.userChoiceRepository.findUnique(tx, { userId: choiceDto.userId!, choiceId: choice.id });

        if (!existingUserChoice) await this.userChoiceRepository.create(tx, { userId: choiceDto.userId!, choiceId: choice.id })
