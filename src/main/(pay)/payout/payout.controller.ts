...
export class PayoutController {
    ...
    async getAllPayouts(@Query() query: PayoutQueryDto): Promise<PayoutResponseDto[]> {
    ...
    async getPendingPayouts(@Query("limit") limit?: number): Promise<PayoutResponseDto[]> {
    ...
    async countPayouts(@Query() query: PayoutQueryDto): Promise<{ count: number }> {
    ...
    async deletePayout(@GetUser() user: TUser, @Param("id") id: string): Promise<void> {
    ...
    async validateOwnership(
        ...
        const isOwner = await this.payoutService.validatePayoutOwnership(id, user.userId);
        return { isOwner };
    }
