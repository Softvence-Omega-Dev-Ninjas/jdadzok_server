...
export class RevenueController {
  ...
  async getUserRevenueHistory(
    ...
    try {
      const result = await this.revenueService.getUserRevenueHistory(
        userId,
        queryDto,
      );
      return {
  ...
  async calculateMonthlyRevenue(
    ...
    try {
      const result =
        await this.revenueService.calculateMonthlyRevenue(calculationDto);
      return {
        success: true,
  ...
  async createRevenueShare(
    @Body(ValidationPipe) createDto: CreateRevenueShareDto,
  ) {
    try {
      const revenueShare =
        await this.revenueService.createRevenueShare(createDto);
      return {
        success: true,
  ...
  @ApiOperation({
    ...
    description:
      "Returns detailed analytics about platform revenue distribution and trends",
  })
  @ApiResponse({
    status: HttpStatus.OK,
  ...
  async getMonthlyDistribution(
    ...
    try {
      ...
      const allShares = await this.revenueService.getUserRevenueHistory(
        "*",
        queryDto,
      );
  ...
  @ApiOperation({
    ...
    description:
      "Exports revenue shares data for a specified period in CSV format",
  })
  ...
  async getRevenueForecast(@Query("months") months: number = 3) {
    try {
      ...
      const averageRevenue =
        ...
        recentTrend.length;
      const averageUsers =
        recentTrend.reduce((sum, month) => sum + month.activeUsers, 0) /
        recentTrend.length;
      ...
      const forecast = Array.from({ length: months }, (_, index) => {
        ...
        return {
          ...
          projectedUsers: Math.round(
            averageUsers * (1 + Math.random() * 0.15 - 0.075),
          ), // ±7.5% variance
  ...
  async validateRevenueCalculation(
    ...
    try {
      ...
      const result =
        await this.revenueService.calculateMonthlyRevenue(validationDto);
