...
export class OptService {
    ...
    async generateOtp(
        ...
        const existing = await this.redisService.get<OtpRedisData>(redisKey, userId);

        if (existing)
            throw new ForbiddenException('You can request a new OTP after some time.');

        const token = uniqueID({ length, alphabet: true });
        const expireAt = new Date(Date.now() + (typeof ttl === 'string' ? TTL[ttl] : ttl)).toISOString();
    ...
    async verifyOtp(input: OtpVerifyPayload, isDelete: boolean = true): Promise<boolean> {
        ...
        if (data.token !== token) {
            ...
            }, new Date(data.expireAt).getTime() - Date.now(), userId); // Keep the remaining TTL
    ...
    async getToken(payload: Omit<OtpVerifyPayload, "token">) {
        ...
        const data = await this.redisService.get<OtpRedisData>(redisKey, payload.userId);
        return data
    }
