...
export class VolunteerTrackingService {
    ...
    async updateVolunteerHours(updateDto: UpdateVolunteerHoursDto): Promise<UserMetrics> {
        ...
        try {
            ...
            await this.logVolunteerActivity(userId, hours, projectId, workDescription, workDate);
            ...
            this.logger.error(`Failed to update volunteer hours: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async updateApplicationStatus(
        ...
        try {
            ...
            if (status === "ACCEPTED") {
                await this.handleApplicationAcceptance(application.userId, application.projectId);
            }
    ...
    async verifyServiceCompletion(
        ...
        try {
            ...
            if (actualWeeks < 8) {
                throw new BadRequestException("Service period must be at least 8 weeks");
            }
            ...
            await this.storeServiceCompletionRecord(serviceRecord, verifiedBy, serviceSummary);
            ...
            this.logger.error(`Failed to verify service completion: ${error.message}`, error.stack);
            throw error;
        }
    }
     ...
     */
    async getUserVolunteerSummary(userId: string): Promise<UserVolunteerSummaryDto> {
        try {
            const user = await this.prisma.user.findUnique({
                where: { id: userId },
                include: {
            ...
            const hasCompleted8WeekService = await this.hasUserCompleted8WeekService(userId);
     ...
     */
    async getProjectVolunteerStats(projectId: string): Promise<ProjectVolunteerStatsDto> {
        try {
            const project = await this.prisma.volunteerProject.findUnique({
                where: { id: projectId },
                include: {
            ...
            const activeVolunteers = applications.filter((app) => app.status === "ACCEPTED").length;
            ...
            this.logger.error(`Failed to get project volunteer stats: ${error.message}`);
            throw error;
        }
    }
    ...
    async getVolunteerLeaderboard(limit: number = 50) {
        try {
            ...
            this.logger.error(`Failed to get volunteer leaderboard: ${error.message}`);
            throw error;
        }
    }
    ...
    async getPlatformVolunteerStats(): Promise<PlatformVolunteerStatsDto> {
        try {
            ...
            const averageHoursPerUser = totalVolunteers > 0 ? totalHours / totalVolunteers : 0;
            ...
            this.logger.error(`Failed to get platform volunteer stats: ${error.message}`);
            throw error;
        }
    }
     ...
     */
    async processBatchVolunteerHoursUpdate(batchDto: BatchVolunteerHoursUpdateDto): Promise<{
        successful: number;
        failed: number;
        errors: string[];
    }> {
        ...
        try {
            ...
            this.logger.error(`Batch volunteer hours update failed: ${error.message}`, error.stack);
            throw error;
        }
    }
     ...
     */
    private async checkCapLevelEligibilityAfterVolunteerUpdate(userId: string): Promise<void> {
        try {
            ...
            this.logger.error(`Failed to check cap level eligibility: ${error.message}`);
        }
    }
    ...
    private async check8WeekServiceCompletion(userId: string): Promise<void> {
        ...
        if (user.metrics.volunteerHours >= 200 && user.metrics.completedProjects >= 3) {
     ...
     */
    private async handleApplicationAcceptance(userId: string, projectId: string): Promise<void> {
        // Log application acceptance - could trigger notifications here
        this.logger.log(`User ${userId} application accepted for project ${projectId}`);
    }
    ...
    private async getServiceCompletionDate(userId: string): Promise<Date | undefined> {
