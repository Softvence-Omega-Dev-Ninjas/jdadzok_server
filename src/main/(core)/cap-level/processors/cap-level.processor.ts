...
export class CapLevelProcessor extends WorkerHost {
  ...
  async process(job: Job<any, any, string>) {
    ...
    try {
      ...
      this.logger.error(
        `Job processing failed: ${job.name} (ID: ${job.id})`,
        error.stack,
      );
      throw error;
    }
  ...
  private async processUserEligibility(job: Job<UserEligibilityJobData>) {
    ...
    return {
      ...
      autoPromotionQueued:
        ...
        eligibility.requirements?.requiresVerification === false,
    };
  }
   ...
   */
  private async processUserPromotion(
    job: Job<UserPromotionJobData>,
  ): Promise<any> {
    ...
    this.logger.log(
      `Successfully promoted user ${userId} to ${promotedUser.capLevel}`,
    );
   ...
   */
  private async processActivityScoreRecalculation(
  ...
  private async processBatchPromotion(job: Job<BatchPromotionJobData>) {
    ...
    const result =
      await this.capLevelService.processPendingPromotions(capLevel);
   ...
   */
  private async processBatchMetricsRecalculation(
    job: Job<BatchMetricsJobData>,
  ) {
    const {
      userIds,
      capLevel,
    ...
    this.logger.log(
      `Processing batch metrics recalculation (batch size: ${batchSize})`,
    );
    this.logger.log(
  ...
  private async processVolunteerHours(job: Job<VolunteerHoursJobData>) {
    ...
    this.logger.log(
      `Processing volunteer hours for user ${userId}: ${hours} hours`,
    );

    const updatedMetrics =
      await this.volunteerTrackingService.updateVolunteerHours({
        userId,
        hours,
        projectId,
        workDescription,
   ...
   */
  private async processServiceCompletionCheck(
    job: Job<{ userId: string }>,
  ): Promise<any> {
    const { userId } = job.data;
  ...
  onProgress(job: Job, progress: number) {
    this.logger.debug(
      `Job progress: ${job.name} (ID: ${job.id}) - ${progress}%`,
    );
  }
}
