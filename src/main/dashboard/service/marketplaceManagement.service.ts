...
export class MarketplaceManagementService {
    ...
    //                   seller: { profile: { name: { contains: search, mode: Prisma.QueryMode.insensitive } } },
    //                 },
    //               ],
    //             }
    //           : undefined,
    ...
    //         const completedPayments = (o.payments ?? []).filter((pm) => pm.status === 'COMPLETED');
    //         if (completedPayments.length > 0) {
    //           salesUnits += o.quantity ?? 0;
    //           totalSalesAmount += completedPayments.reduce((s, pm) => s + (pm.amount ?? 0), 0);
    //         }
    //       }
    ...
    //         category: p.category?.title ?? p.category?.name ?? null,
    //         price: p.price,
    //         stock,
    //         sales: salesUnits,
    //         totalSalesAmount,
