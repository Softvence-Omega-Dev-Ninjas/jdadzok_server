...
export class FeedService {
    constructor(private readonly choiceRepository: ChoicesRepository, private readonly userRepository: UserRepository, private readonly postRepository: PostRepository) { }
    ...
    async generateUserFeed(userId: string): Promise<Post[]> {
        ...
        const scoredPosts = posts && posts.map(post => ({
            post,
            score: feedAlgorithm.scorePost({
                ...post,
                author: post.author,
            }, userId, choiceSlugs, following)
        }));
