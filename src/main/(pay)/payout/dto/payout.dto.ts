...
class PayoutDto {
  ...
  @ApiProperty({ enum: PaymentMethod, description: "Payment method for payout" })
...
export class UpdatePayoutDto extends PartialType(PayoutDto) {
  @ApiProperty({ enum: PayOutStatus, description: "Payout status", required: false })
  @IsOptional()
  @IsEnum(PayOutStatus)
  status?: PayOutStatus;
...
export class PayoutQueryDto {
  ...
  @ApiProperty({ enum: PayOutStatus, description: "Payout status to filter", required: false })
  @IsOptional()
  @IsEnum(PayOutStatus)
  status?: PayOutStatus;

  @ApiProperty({ enum: PaymentMethod, description: "Payment method to filter", required: false })
  @IsOptional()
  @IsEnum(PaymentMethod)
  method?: PaymentMethod;

  @ApiProperty({ description: "Minimum amount filter", minimum: 0, required: false })
  @IsOptional()
  @IsNumber()
  @Min(0)
  @Type(() => Number)
  ...
  @ApiProperty({ description: "Maximum amount filter", minimum: 0, required: false })
  @IsOptional()
  @IsNumber()
  @Min(0)
  @Type(() => Number)
  ...
  @ApiProperty({ description: "Limit number of results", minimum: 1, maximum: 100, required: false })
  @IsOptional()
  @IsNumber()
  @Min(1)
  @Type(() => Number)
  ...
  @ApiProperty({ description: "Offset for pagination", minimum: 0, required: false })
  @IsOptional()
  @IsNumber()
  @Min(0)
  @Type(() => Number)
...
export class ProcessPayoutDto {
  @ApiProperty({ description: "External transaction ID from payment processor" })
  @IsString()
  transactionId: string;
